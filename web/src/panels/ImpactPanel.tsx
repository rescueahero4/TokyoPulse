...
export const STATUS_COLOR: Record<Impact['status'], string> = {
  normal: '#00ff41',
  delay: '#ffaa00',
  suspended: '#ff3333',
  unknown: '#6b7280',
...
/**
 * The impact detail with no panel chrome of its own — status, flood count,
 * wards, stations, events.
 *
 * Rendered both by `ImpactPanel` (standalone, kept for the frozen contract) and
 * inline by `LinePanel`, which merges the line search and its detail into one
 * widget. Same code either way, so the two can't drift.
 */
export function ImpactBody(p: {
  impact: Impact;
  loading: boolean;
...
/**
 * Standalone impact panel — kept exported at its frozen signature.
 * The HUD now renders this detail inline inside `LinePanel`.
 */
export function ImpactPanel(p: {
  ...
  return (
    <div className={`tp-panel tp-impact-panel${collapsed ? ' tp-panel-collapsed' : ''}`}>
      ...
      />
      {collapsed ? null : (
        <ImpactBody impact={impact} loading={p.loading} onStationClick={p.onStationClick} />
      )}
    </div>
  );
}
